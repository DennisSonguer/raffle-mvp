  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      try {
        ...
        // sorgt dafür, dass jede Raffle genau 1 RUNNING Runde hat (Server-seitig)
        await Promise.all(rs.map((r) => ensureRunningRound(r)));

        const rr = await getRunningRounds(rs.map((x) => x.id));
        setRounds(rr);
        ...
        console.error("raffles load error", e);
        setErr("Fehler beim Laden. Bitte Seite neu laden.");
      } finally {
    ...
    const t2 = setInterval(loadAll, 4000);
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <a href="/admin" className="text-sm font-semibold text-white underline">
                Admin
              </a>

              {user ? (
                <span className="text-sm text-zinc-300">
                  User: <span className="font-semibold text-white">{user}</span>
                </span>
              ) : (
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="text-zinc-300">
                      Ticket:{" "}
                      <span className="font-semibold text-white">{r.ticketPrice}</span>
                    </div>
                  ...
                  <div className="mt-3 text-sm text-zinc-400">
                    Deine Tickets hier:{" "}
                    <span className="font-semibold text-zinc-200">{my}</span>
                  </div>

                  {left <= 0 && (
                    <div className="mt-3 text-xs text-zinc-400">
                      Ziehung läuft… (wird automatisch aktualisiert)
                    </div>
                  )}
                </div>
              </div>
